#' Dictionary subset
#'
#' Extracts the names of dictionaries, optionally subsetted by type (mean, SD, COV) or component (identities, behaviors, mods, settings)
#'
...
#' Equation subset
#'
#' Extracts the names of equations, optionally subsetted by gender
#'
...
eqn_subset <- function(eqns, gender = NA){
  ...
  for(eqn in eqns){
    if(gender %in% dict@gender | is.na(gender)){
